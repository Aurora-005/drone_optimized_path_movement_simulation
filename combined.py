...
def neighbors(node):
    x, y = node
    potential_neighbors = [(x + 1, y), (x - 1, y), (x, y + 1)]
    return [n for n in potential_neighbors if 0 <= n[0] < grid_size and 0 <= n[1] < grid_size]

# Bellman-Ford update for new obstacles with negative edge detection
def bellman_ford_update(new_obstacle):
    if new_obstacle not in obstacles:
        obstacles[new_obstacle] = -2  # Adding a negative edge weight for detection
    for neighbor in neighbors(new_obstacle):
        if neighbor in obstacles:
            obstacles[neighbor] += 2  # Increase cost if already an obstacle
        else:
            obstacles[neighbor] = 2  # Set initial cost if not in obstacles
...
while running:
    ...
    if current_step < len(path):
        ...
        if current_step == 2 and not obstacle_added:
            ...
            obstacles[new_obstacle] = -1  # Add a new obstacle with a negative weight to simulate detection
